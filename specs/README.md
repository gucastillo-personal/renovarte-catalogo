...
| [0016](./0016-chat-recomendador-cremas/spec.md) | Chat conversacional embebido ("Colibrí"): a partir de tipo de piel + presupuesto, recomienda 3 combos de cremas (más barato/medio/premium), cada combo un paquete de 2+ productos reales del catálogo publicado; "más barato"/"medio" respetan el presupuesto como techo, "premium" puede superarlo hasta 20%; runtime (websocket + LLM/RAG) vive en 2 proyectos nuevos fuera de `renovarte-catalogo`, que se mantiene 100% estático — resuelve sin enmienda el conflicto con `constitution.md §II.4`; `renovarte-pipeline` no se toca (tipo de piel inferido sobre campos de texto libre ya públicos); techo de gasto de la API de Claude USD 20/mes con auto-deshabilitación (RNF-09) — sin preguntas abiertas, ejercicio de aprendizaje explícito (no optimización de ventas), lista para pasar a diseño/RFC | Backlog |
...
| RF-14 | Chat conversacional embebido ("Colibrí"): tipo de piel + presupuesto → 3 combos de cremas (más barato/medio/premium, cada uno 2+ productos reales) con productos reales del catálogo, nunca inventados; "más barato"/"medio" ≤ presupuesto, "premium" ≤ presupuesto × 1.20 | 0016 | Backlog — conflicto con `constitution.md §II.4` resuelto (runtime nunca vive en `renovarte-catalogo`, ver RNF-08); sin preguntas abiertas (techo de costo de la API de Claude resuelto vía RNF-09), lista para pasar a diseño/RFC |
| RNF-06 | El chat no expone credenciales de terceros ni costo/margen real | 0016 | Backlog |
| RNF-07 | El catálogo (RF-01 a RF-04) sigue funcionando si el chat no está disponible | 0016 | Backlog |
| RNF-08 | El runtime del chat (transporte en vivo, conexión al LLM, RAG) vive exclusivamente fuera de `renovarte-catalogo`; este repo nunca aloja backend/runtime propio | 0016 | Backlog |
| RNF-09 | Techo de gasto mensual de la API del LLM (USD 20/mes); al alcanzarlo, el chat se deshabilita automáticamente y el resto del catálogo sigue funcionando sin degradación | 0016 | Backlog — excepción explícita y acotada al chat al invariante "$0 infra" (RNF-01 / `constitution.md §II.5`), aprobada por el CTO/CEO |
